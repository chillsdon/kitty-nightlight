...
#define NUMPIXELS1      68 // number of LEDs on strip
...
unsigned long intervals [] = { 20, 20, 75, 100, 100, 100, 30 } ; // speed for each pattern - add here when adding more cases 
int fadeStep = 0; // stste variable for fade function
int numberOfCases = 6; // how many case statements or patterns you have
const byte button = 3; // pin to connect button switch to between pin and ground

void setup() {
  strip.begin(); // This initializes the NeoPixel library.
  wipe(); // wipes the LED buffers
  pinMode(button, INPUT_PULLDOWN); // change pattern button
}
